...
class Search {
  ...
  constructor({ form, input, list, resultTitle, resultTitleTemplate }) {
    ...
    // 初始化模糊匹配状态 (仅在有筛选面板时)
    this.fuzzyToggle = document.getElementById(
      "fuzzy-search-toggle",
    ) as HTMLInputElement;
    ...
    if (this.fuzzyToggle && this.searchFilters) {
      // 从本地存储读取用户的设置
      const savedSetting = localStorage.getItem(this.STORAGE_KEY);
      this.isFuzzyMatch = savedSetting === "true";
      ...
      this.fuzzyToggle.addEventListener("change", () => {
        ...
        // 保存到本地存储
        localStorage.setItem(this.STORAGE_KEY, this.isFuzzyMatch.toString());
    ...
    for (const item of rawData) {
      ...
      for (const keyword of keywords) {
        ...
        for (let i = 0; i < titleWords.length; i++) {
          ...
          // 跳过太短的词（1-2 个字符）
          if (titleWord.length < 2) continue;

          const similarity = Search.calculateSimilarity(titleWord, keyword);
          if (similarity >= similarityThreshold) {
            const startIndex = item.title.indexOf(titleWord);
            if (startIndex !== -1) {
              titleMatches.push({
                start: startIndex,
                end: startIndex + titleWord.length,
              });
              totalSimilarity += similarity * 1.5; // 标题匹配权重更高
              matchCount++;
            }
          }
        }
        ...
        for (let i = 0; i < contentWords.length; i++) {
          ...
          // 跳过太短的词和常见虚词
          if (contentWord.length < 2) continue;

          const similarity = Search.calculateSimilarity(contentWord, keyword);
          if (similarity >= similarityThreshold) {
            const startIndex = item.content.indexOf(contentWord);
            if (startIndex !== -1) {
              contentMatches.push({
                start: startIndex,
                end: startIndex + contentWord.length,
              });
