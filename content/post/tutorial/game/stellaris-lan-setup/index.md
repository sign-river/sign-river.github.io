...
虽然市面上有现成的加速器，但效果参差不齐且价格不菲。今天，我们将手把手教你搭建一个专属于你和小伙伴的联机加速节点。

本方案基于 OpenVPN （TAP 模式），通过去除加密环节来极致优化传输速度，实测 10M 带宽即可流畅支撑 3-4 人联机。

特别致谢： 本教程方案核心及加速器客户端由大佬 Dogfight360 免费提供，教程基础内容由 唏噓南溪 编写。本文旨在对原教程进行细化与解读，帮助新手更轻松地上手。 \* 原作者博客：[Dogfight360 Blog](https://www.dogfight360.com/blog/1590/#comment-41142)
...
群星联机的数据交换量较大。根据实测经验，10Mbps 的带宽大约能支撑 3-4 车人同时游玩，一车大概 10 来人左右。带宽越大，后期卡顿的概率越低。
...
- 情况 A：版本低于 2.5 （例如 2.4.x)
  这是 CentOS 7 默认的旧版本，与本教程提供的原始配置文件完全兼容，您在后续章节无需进行额外修改。

- 情况 B：版本为 2.5 或 2.6+
  现在的云服务器系统（如新版 CentOS 或 Debian/Ubuntu）通常会安装较新的版本。
  如果您的版本是 2.5 或更高，请务必记住这一点。因为新版本废弃了旧的加密参数，我们在下一章部署时，必须手动修改配置文件（添加 data-ciphers 等设置）才能正常连接，否则会导致无法联机。
...
如果输出如下图所示，说说明你的 firewalld 防火墙服务在正常运行，我们需要继续操作，放行端口 3074(UDP) 和 3075(TCP)
...
**这是最容易被新手忽略的一步！**
`checkpsw.sh` 是一个 Shell 脚本，OpenVPN 需要调用它来验证客户端传来的账号密码。如果它没有“可执行权限”，服务器就会拒绝所有连接请求，导致报错。
...
<img src="images/2026-02-11-21-44-33.png" alt="image" width="1000"> 4. 针对 OpenVPN 2.6.x （最新版）
...
- 如果显示 Exiting due to fatal error 或其他报错，请仔细检查报错信息（通常是 cipher 或 auth 参数写错了），并返回第三章重新修改。
...
- 注意：命令末尾的 & 符号绝对不能漏！它代表“在后台运行”。如果漏掉，服务器重启后会卡在启动画面进不去系统。
...
这是很多教程容易漏掉的一步。在 CentOS 7 中，rc.local 默认没有执行权限。如果不执行此命令，开机启动将无效：
...
- IP：修改为您服务器的 **公网 IP 地址**。
- TCP Port / UDP Port：保持 3075 和 3074 （除非您在服务端修改了端口）。
- USER：修改为 **4.3 设置连接账号密码** 中您在 psw-file 里设置的用户名。
- PASS：修改为 **4.3 设置连接账号密码\*** 中您在 psw-file 里设置的密码。
...
<img src="images/2026-02-11-22-03-17.png" alt="image" width="500">
...
- 如果一直卡在“正在连接”或提示验证失败，请检查防火墙端口（第一章）或账号密码（第三章）。
  <img src="images/2026-02-11-22-03-31.png" alt="image" width="1000">

当所有小伙伴都显示“连接状态：正常”后，大家实际上已经处于同一个虚拟局域网中。
<img src="images/2026-02-11-22-03-48.png" alt="image" width="1000">

---

至此，您的专属群星联机加速节点已部署完毕。联机时，只需所有玩家启动客户端并连接同一节点，即可直接进入游戏，享受低延迟的联机体验！
