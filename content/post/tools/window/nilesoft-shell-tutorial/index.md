...
Nilesoft Shell 默认的菜单都是英文的，使用起来可能不太习惯。这里提供一个汉化版的文件管理菜单配置，可以直接替换使用。

### 汉化的菜单功能

汉化后的菜单包含以下实用功能：

- 📋 **复制路径**：复制文件/文件夹的完整路径、文件名、扩展名等
- ✏️ **修改扩展名**：快速修改文件扩展名
- ✅ **选择操作**：全选、反选、取消选择
- 🔐 **获取所有权**：以管理员权限获取文件/文件夹所有权
- 👁️ **显示/隐藏**：切换系统文件和文件扩展名显示
- 📝 **文件属性**：快速修改文件的隐藏、系统、只读等属性
- 📦 **注册服务器**：注册/取消注册 DLL/OCX 文件
- ➕ **新建文件/文件夹**：快速创建带时间戳的文件夹和 15+ 种常用格式文件（含 Markdown、Python、JavaScript、CSS、JSON、YAML、CSV、HTML、INI、ENV、BAT、PowerShell 等）

### 替换步骤

**第一步**：打开配置文件

进入 Nilesoft 安装目录，找到 `imports` 文件夹，打开 `file-manage.nss` 文件。

**第二步**：替换内容

删除文件中的所有内容，将下面的汉化代码**完整复制**并粘贴进去：
...
1. 按 `Ctrl + S` 保存文件
2. 重启资源管理器（运行 Nilesoft Shell 程序，选择 **Restart Explorer**）

### 💡 提示

- 这只是默认配置中的一个模块，其他模块（如终端、开发工具等）的汉化可以用同样的方法
- 如果想恢复英文，只需重新安装 Nilesoft Shell 或从官网下载原始配置文件
- 建议在修改前备份原始的 `file-manage.nss` 文件
